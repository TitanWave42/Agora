...
EventData DoFFT::Launch(size_t tag) {
  ...
  if ((kPrintPilotCorrStats == true) &&
    ...
    std::vector<std::complex<float>> pilot_corr =
        CommsLib::CorrelateAvx(samples_vec, mac_sched_->PilotCf32());
    ...
    size_t seq_len = mac_sched_->PilotCf32().size();
  ...
  for (size_t sc_block_idx = 0; sc_block_idx < num_sc_blocks; sc_block_idx++) {
    ...
    for (size_t sc_j = 0; sc_j < kTransposeBlockSize;
      ...
      if (symbol_type == SymbolType::kPilot) {
        __m512 pilot_tx = _mm512_set_ps(mac_sched_->PilotsSgn()[sc_idx + 7].im,
                                        mac_sched_->PilotsSgn()[sc_idx + 7].re,
                                        mac_sched_->PilotsSgn()[sc_idx + 6].im,
                                        mac_sched_->PilotsSgn()[sc_idx + 6].re,
                                        mac_sched_->PilotsSgn()[sc_idx + 5].im,
                                        mac_sched_->PilotsSgn()[sc_idx + 5].re,
                                        mac_sched_->PilotsSgn()[sc_idx + 4].im,
                                        mac_sched_->PilotsSgn()[sc_idx + 4].re,
                                        mac_sched_->PilotsSgn()[sc_idx + 3].im,
                                        mac_sched_->PilotsSgn()[sc_idx + 3].re,
                                        mac_sched_->PilotsSgn()[sc_idx + 2].im,
                                        mac_sched_->PilotsSgn()[sc_idx + 2].re,
                                        mac_sched_->PilotsSgn()[sc_idx + 1].im,
                                        mac_sched_->PilotsSgn()[sc_idx + 1].re,
                                        mac_sched_->PilotsSgn()[sc_idx].im,
                                        mac_sched_->PilotsSgn()[sc_idx].re);
      ...
      if (symbol_type == SymbolType::kPilot) {
        __m256 pilot_tx0 = _mm256_set_ps(mac_sched_->PilotsSgn()[sc_idx + 3].im,
                                         mac_sched_->PilotsSgn()[sc_idx + 3].re,
                                         mac_sched_->PilotsSgn()[sc_idx + 2].im,
                                         mac_sched_->PilotsSgn()[sc_idx + 2].re,
                                         mac_sched_->PilotsSgn()[sc_idx + 1].im,
        ...
        __m256 pilot_tx1 =
            _mm256_set_ps(mac_sched_->PilotsSgn()[sc_idx + 7].im,
                          mac_sched_->PilotsSgn()[sc_idx + 7].re,
                          mac_sched_->PilotsSgn()[sc_idx + 6].im,
                          mac_sched_->PilotsSgn()[sc_idx + 6].re,
                          mac_sched_->PilotsSgn()[sc_idx + 5].im,
