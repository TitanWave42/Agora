...
EventData DoDecode::Launch(size_t tag) {
  ...
  const size_t num_bytes_per_cb = mac_sched_->NumBytesPerCb(Direction::kUplink);
  ...
  int8_t* llr_buffer_ptr =
      ...
      (mac_sched_->ModOrderBits(Direction::kUplink) *
  ...
  if ((kEnableMac == false) && (kPrintPhyStats == true) &&
    ...
    for (size_t i = 0; i < num_bytes_per_cb; i++) {
      ...
      auto tx_byte = static_cast<uint8_t>(
          mac_sched_->GetInfoBits(mac_sched_->UlBits(), Direction::kUplink,
                                  symbol_idx_ul, ue_id, cur_cb_id)[i]);
