...
LdpcUpdater::LdpcUpdater(uint16_t ul_base_graph, bool ul_early_term,
...
void LdpcUpdater::UpdateLdpc(LDPCconfig ldpc_config, Direction dir,
                             size_t mcs_index) {
  switch (dir) {
    case Direction::kUplink:
      UpdateUlLdpcConfig(ldpc_config, mcs_index);
      break;
    case Direction::kDownlink:
      UpdateDlLdpcConfig(ldpc_config, mcs_index);
        break;
    default: //Assuming default is control because there is no control dicection, but this is non optimal.
      UpdateCtrlMCS(ldpc_config, mcs_index); //This function would be more aptly named UpdateCtrlLdpcConfig.
  }
}

std::vector<pair<UlMcsParams*, DlMcsParams*> ul_dl_ldpc_table;
...
LDPCconfig LdpcUpdater::UpdateUlLdpcConfig(size_t ul_mcs_index) {
  ...
  LDPCconfig LdpcUpdater::UpdateCtrlMCS(size_t control_mcs_index) {
    if (this->frame_.NumDlControlSyms() > 0) {
      ...
      size_t bcast_zc = SelectZc(bcast_base_graph, dl_bcast_code_rate,
                                 dl_bcast_mod_order_bits_, ofdm_ctrl_data_,
                                 kCbPerSymbol, "downlink broadcast");

      // Always positive since dl_code_rate is smaller than 1
      size_t bcast_num_rows = static_cast<size_t>(std::round(
                                  1024.0 * LdpcNumInputCols(bcast_base_graph) /
                                  dl_bcast_code_rate)) -
      ...
      LDPCconfig dl_bcast_ldpc_config_ = LDPCconfig(
          bcast_base_graph, bcast_zc, max_decoder_iter, true, bcast_num_cb_len,
          bcast_num_cb_codew_len, bcast_num_rows, 0);
      ...
      AGORA_LOG_INFO(
          ...
          dl_bcast_ldpc_config_.NumBlocksInSymbol(),
          dl_bcast_ldpc_config_.NumCbLen(),
          dl_bcast_ldpc_config_.NumCbCodewLen(),
  ...
  void LdpcUpdater::CalculateUlLdpcProperties(UlMcsParams* ul_mcs_params) {
    ...
    ul_mcs_params->ul_data_bytes_num_persymbol_ =
        ul_mcs_params->ul_num_bytes_per_cb_ *
        ul_mcs_params->NumBlocksInSymbol();
    uul_mcs_params->ul_mac_packet_length_ =
        ul_mcs_params->ul_data_bytes_num_persymbol_;

    const size_t ul_ldpc_input_min =
        (((ul_mcs_params->ul_ldpc_config_.NumCbLen() / ul_mcs_params->ul_ldpc_config_.ExpansionFactor()) -
          1) *
             (ul_mcs_params->ul_ldpc_config_.ExpansionFactor() / 8) +
         32);
    const size_t ul_ldpc_sugg_input = LdpcEncodingInputBufSize(
        ul_mcs_params->ul_ldpc_config_.BaseGraph(), ul_mcs_params->ul_ldpc_config_.ExpansionFactor());

    if (ul_ldpc_input_min >
        (ul_mcs_params->ul_num_bytes_per_cb_ + ul_mcs_params->ul_num_padding_bytes_per_cb_)) {
      ...
      AGORA_LOG_WARN(
          ...
          ul_num_padding_bytes_per_cb_, increased_padding, ul_mcs_params->ul_num_bytes_per_cb_,
          ul_ldpc_input_min);
...
void LdpcUpdater::CalculateDlLdpcProperties(DlMcsParams* dl_mcs_params) {
    ...
    dl_mcs_params->dl_num_bytes_per_cb_ = dl_mcs_params->dl_ldpc_config_.NumCbLen() / 8;
    dl_mcs_params->dl_num_padding_bytes_per_cb_ =
        Roundup<64>(dl_mcs_params->dl_num_bytes_per_cb_) - dl_mcs_params->dl_num_bytes_per_cb_;
    dl_mcs_params->dl_data_bytes_num_persymbol_ =
        dl_mcs_params->dl_num_bytes_per_cb_ * dl_ldpc_config_.NumBlocksInSymbol();
    dl_mcs_params->dl_mac_packet_length_ = dl_mcs_params->dl_data_bytes_num_persymbol_;
    // Smallest over the air packet structure
    ...
    dl_mcs_params->dl_mac_data_bytes_num_perframe_ =
        dl_mcs_params->dl_mac_data_length_max_ * dl_mcs_params->dl_mac_packets_perframe_;
    dl_mcs_params->dl_mac_bytes_num_perframe_ =
        dl_mcs_params->dl_mac_packet_length_ * dl_mcs_params->dl_mac_packets_perframe_;

    //((cb_len_bits / zc_size) - 1) * (zc_size / 8) + kProcBytes(32)
    const size_t dl_ldpc_input_min =
        (((dl_mcs_params->dl_ldpc_config_.NumCbLen() / dl_mcs_params->dl_ldpc_config_.ExpansionFactor()) -
          1) *
             (dl_mcs_params->dl_ldpc_config_.ExpansionFactor() / 8) +
         32);
    const size_t dl_ldpc_sugg_input = LdpcEncodingInputBufSize(
        dl_mcs_params->dl_ldpc_config_.BaseGraph(), dl_mcs_params->dl_ldpc_config_.ExpansionFactor());

    if (dl_ldpc_input_min >
        (dl_mcs_params->dl_num_bytes_per_cb_ + dl_mcs_params->dl_num_padding_bytes_per_cb_)) {
      // Can cause a lot of wasted space, specifically the second argument of the max
      const size_t increased_padding =
