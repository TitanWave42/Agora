...
TEST(TestControl, VerifyCorrectness) {
  ...
  msg.push_back(1051);
  mac_scheduler->GenBroadcastSlots(data_buffer, msg);
  ...
  auto decoded_msg = mac_scheduler->DecodeBroadcastSlots(
      reinterpret_cast<int16_t*>(data_buffer[0]));
